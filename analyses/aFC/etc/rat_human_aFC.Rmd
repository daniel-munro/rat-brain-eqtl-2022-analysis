...
# eqtls <- read_tsv("../data/eqtls.txt", col_types = "cc----d-d-") %>%
#     rename(log2_aFC = log2_aFC_eQTL) %>%
#     filter(qval < 0.05,
#            !is.na(log2_aFC))
...
# write_tsv(gene_map, "gene_map.txt")
gene_map <- read_tsv("gene_map.txt", col_types = "cc")
...
gene_afc <- both %>%
    group_by(gene_id) %>%
    # summarise(log2_aFC = list(log2_aFC))
