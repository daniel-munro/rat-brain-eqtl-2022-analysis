...
```{r}
counts <- read_tsv("../../data/mixups/test_snps.counts.txt.gz", col_types = "ccii")
...
vcf <- read_tsv("../../data/mixups/test_snps.vcf.gz", comment = "##",
...
```{r}
matches <- bam %>%
    left_join(vcf, by = "variant_id") %>%
    group_by(sample_id, vcf_id) %>%
...
```{r}
# matches %>%
#     ggplot(aes(x = vcf_id, y = sample_id, fill = similarity)) +
#     geom_tile() +
...
```{r}
# # Plot per tissue for presentation
# tiss <- "VoLo"
# matches %>%
#     separate(sample_id, c("bam_id", "tissue")) %>%
#     filter(tissue == tiss) %>%
...
#     theme(axis.text.x = element_text(hjust = 1, vjust = 0.5, angle = 90),
#           legend.position = "top") +
#     ylab("RNA-Seq/GBS genotype similarity") +
#     ggtitle(tiss)
# ggsave(str_glue("frac_snps_match.{tiss}.png"), width = 10, height = 4)
```
...
vcf_all <- read_tsv("../../data/mixups/all_rats_50_test_snps.vcf.gz",
...
matches_all <- bam %>%
    ...
    left_join(vcf_all, by = "variant_id") %>%
    group_by(sample_id, vcf_id) %>%
    summarise(similarity = mean(1 - abs(bam_geno - vcf_geno)),
        # similarity = mean(bam_geno == vcf_geno),
              .groups = "drop")
```
...
<!-- These three are higher than the rest, but nothing looks conclusive (and it could just be that these two rats are especially related): -->
